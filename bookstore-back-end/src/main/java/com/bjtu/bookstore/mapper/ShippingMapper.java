...
public interface ShippingMapper {
    ...
    //添加地址信息
    @Insert("<script>"
            +"insert into shipping (userId,consigneeName,address ,phone) values (#{userId},#{consigneeName},#{address},#{phone})"
            +"</script>"
    )
    ...
    @Update("<script>"
            ...
            +"</set>"
            +"where userId=#{userId} and id=#{shippingId}"
            +"</script>"
    )
    ...
    @Delete("<script>"
                   ...
                   " where consigneeName= #{consigneeName} and address=#{address} and phone=#{phone}"
