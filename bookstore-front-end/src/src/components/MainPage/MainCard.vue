        ...
        <el-card class="card1">
          <el-row class="first">
            ...
            <el-col :span="6" :offset="0">
              <div class="svgHover">
                <svg :width='width' :height="height" @mousemove='listener($event)' @click="goToLike">
                  <a :href="tag.href" v-for='tag in tags'>
                    <text :x='tag.x' :y='tag.y' :font-size='20 * (600/(600-tag.z))' :fill-opacity='((400+tag.z)/600)' class="cloud">{{tag.text}}</text>
                  </a>
                </svg>
              </div>
            </el-col>
...
export default {
  ...
  created(){//初始化标签位置
    ...
    this.$API
        ...
        .then((data) => {
          for(let i = 0; i < data.tagList.tagsNum; i++){
            let tag = {};
            let k = -1 + (2 * (i + 1) - 1) / this.tagsNum;
            let a = Math.acos(k);
            let b = a * Math.sqrt(this.tagsNum * Math.PI)//计算标签相对于球心的角度
            tag.text = data.tagList[i].name;
